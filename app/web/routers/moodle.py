...
class CreateAIRequest(BaseModel):
    ...
    folder_name: str = Field(
        default="entrenaí", description="Nombre de la carpeta a crear"
    )
...
async def create_virtual_ai(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Error al crear la inteligencia virtual: {str(e)}"
        )
...
async def create_moodle_folder(
    course_id: int, folder_name: str, moodle_config: dict
) -> int:
    ...
    async with httpx.AsyncClient() as client:
        ...
        response = await client.post(
            f"{moodle_config['url']}/webservice/rest/server.php", params=params
        )

        if response.status_code != 200:
            raise HTTPException(
                status_code=500,
                detail=f"Error al crear carpeta en Moodle: {response.text}",
            )
        ...
        folder_id = (
            result[0]["id"] if isinstance(result, list) and len(result) > 0 else 999
        )
...
async def create_qdrant_collection(collection_name: str, qdrant_config: dict) -> dict:
    ...
    async with httpx.AsyncClient() as client:
        ...
        if response.status_code != 200:
            raise HTTPException(
                status_code=500,
                detail=f"Error al crear colección en Qdrant: {response.text}",
            )
        ...
        try:
            ...
            for node in workflow_template["nodes"]:
                if node["type"] == "@n8n/n8n-nodes-langchain.vectorStoreQdrant":
                    ...
                    node["parameters"]["qdrantCollection"]["cachedResultName"] = (
                        collection_name
                    )
                    # Actualizar el filtro para incluir el ID del curso
                    search_filter = json.loads(
                    ...
                    if "should" in search_filter:
                        for condition in search_filter["should"]:
                            if (
                                "key" in condition
                                and condition["key"] == "metadata.batch"
                            ):
                                condition["match"]["value"] = course_id
                    node["parameters"]["options"]["searchFilterJson"] = json.dumps(
                        search_filter
                    )
            ...
            if response.status_code not in [200, 201]:
                raise HTTPException(
                    status_code=500,
                    detail=f"Error al crear workflow en N8n: {response.text}",
                )
