    ...
    def iniciar(self):
        ...
        try:
            ...
            while not self.terminando:
                ...
                if (
                    self.flujo.hilo_ejecucion
                    and not self.flujo.hilo_ejecucion.is_alive()
                ):
            ...
            try:
                ...
                try:
                    ...
                    modelo_encontrado = any(
                        m.get("name") == modelo_texto for m in modelos
                    )

                    if modelo_encontrado:
                        logger.success(
                            f"Modelo de texto '{modelo_texto}' encontrado en Ollama"
                        )
                    else:
                        logger.warning(
                            f"Modelo '{modelo_texto}' no encontrado en Ollama"
                        )
                        logger.warning(
                    ...
                    if (
                        ...
                        modelo_embedding = configuracion.obtener(
                            "MODELO_EMBEDDING", "all-MiniLM-L6-v2"
                        )
                        modelo_emb_encontrado = any(
                            m.get("name") == modelo_embedding for m in modelos
                        )

                        if modelo_emb_encontrado:
                            logger.success(
                                f"Modelo de embedding '{modelo_embedding}' encontrado en Ollama"
                            )
                        else:
                            logger.warning(
                                f"Modelo de embedding '{modelo_embedding}' no encontrado en Ollama"
                            )

                except Exception as e:
                    logger.warning(f"Error al listar modelos de Ollama: {e}")
                    logger.warning(
                        "El procesamiento puede fallar si los modelos no están disponibles"
                    )
            except ImportError:
                logger.warning(
                    "Ollama está habilitado pero el módulo 'ollama' no está instalado"
                )
...
def main():
    ...
    keep_alive = (
        os.environ.get("BYTEWAX_KEEP_CONTAINER_ALIVE", "false").lower() == "true"
    )
    ...
    if keep_alive:
        ...
        python_file_path = os.environ.get(
