...
def dividir_en_chunks(
    texto: str, tam_chunk: int = 1000, solapamiento: int = 200
) -> List[str]:
    """
...
def generar_embedding(
    ...
    try:
        if usar_ollama:
            ...
            respuesta = ollama.embeddings(model=modelo_nombre, prompt=texto)
...
def procesar_documento_embedding(
    doc_chunk: DocumentoChunk, modelo_nombre: str, usar_ollama: bool = False
) -> Optional[DocumentoEmbedding]:
    """
    ...
    try:
        ...
        embedding = generar_embedding(
            texto=doc_chunk.texto, modelo_nombre=modelo_nombre, usar_ollama=usar_ollama
        )
