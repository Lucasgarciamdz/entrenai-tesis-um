    ...
    if configuracion.obtener("USAR_OLLAMA", "false").lower() == "true":
        try:
            ...
            if modelo_encontrado:
                ...
                logger.warning(f"Puede que necesite descargar el modelo con: ollama pull {modelo_texto}")
            ...
            if configuracion.obtener("USAR_OLLAMA_EMBEDDINGS", "false").lower() == "true":
                modelo_emb = configuracion.obtener("MODELO_EMBEDDING", "all-MiniLM-L6-v2")
                modelo_emb_encontrado = any(m.get("name") == modelo_emb for m in modelos)
                ...
                if modelo_emb_encontrado:
                    ...
                    logger.warning(f"Puede que necesite descargar el modelo con: ollama pull {modelo_emb}")
        except Exception as e:
...
def esperar_y_verificar_resultados(id_documento, max_tiempo=60):
    ...
    while (time.time() - tiempo_inicio) < max_tiempo:
        ...
        logger.info(f"Verificando resultados... ({tiempo_transcurrido:.1f}/{max_tiempo}s)")
        
        # Buscar por el ID original
        try:
            resultados = qdrant.buscar_por_id_original(coleccion, id_documento)
            if resultados:
                logger.success(f"¡Éxito! Se encontraron {len(resultados)} chunks en Qdrant.")
                return resultados
        except Exception as e:
...
def main():
    ...
    if resultados:
        logger.success(f"Flujo completo exitoso. Se encontraron {len(resultados)} chunks en Qdrant.")
        ...
        try:
            ...
            if resultados_busqueda:
                logger.success(f"Búsqueda exitosa. Se encontraron {len(resultados_busqueda)} resultados.")
        ...
        logger.info("- El servicio ByteWax no está procesando correctamente los mensajes")
        ...
        try:
            ...
            for col in colecciones:
                ...
                if nombre == coleccion:
                    ...
                    logger.info(f"La colección '{coleccion}' existe y tiene {puntos} puntos")
                    if puntos == 0:
                        logger.warning("La colección está vacía, no se han insertado documentos")
                    break
            else:
